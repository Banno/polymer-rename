...
class PolymerElementInfo {
  ...
  constructor(domModule, tagName, originalHtml) {
    ...
    treeWalker(this.walkDom).walk(this.domModule,
  ...
  walkDom(element) {
    let childNodes = (element.childNodes || []).filter(child => {
      if (child.tagName && (child.tagName === 'style' || child.tagName ==='script')) {
        return false;
      }
  ...
  visitAttribute(tagLocationInfo, attr) {
    ...
    if (dataBindingInfo) {
      ...
      this.addRenameable(new renameInfo.RenameableProperty(attributeInfo.start,
          attributeInfo.end + 1, attributeInfo.value));
    }
  }
