...
class PolymerElementInfo {
  ...
  visitNodePre(node) {
    if (node.tagName) {
      ...
      let traversalType = traversalTypeByElement[node.tagName] || NodeTraversalType.ALL;
      ...
      if (traversalType === NodeTraversalType.NONE || NodeTraversalType.ATTRIBUTES_ONLY ||
          NodeTraversalType.NON_TEXT_CHILDREN) {
        return;
      }
