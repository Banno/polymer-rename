        ...
        .then(results => {
          ...
          results.features.forEach((featureInfo, fileUrl) => {
            ...
            features.forEach(replacement => {
              ...
              let lineStartIndex = originalFileWorkingContents.substr(0, replacement.start).lastIndexOf('\n');
              if (lineStartIndex < 0) {
                lineStartIndex = 0;
              }
              let currentLineOffset = 0;
              for (let i = originalFileWorkingContents.substr(0, replacement.start).lastIndexOf('\\');
                   i >= lineStartIndex;
                   i = originalFileWorkingContents.substr(0, i - 1).lastIndexOf('\\')) {
                currentLineOffset++;
              }

              newFileContent.unshift(originalFileWorkingContents.substr(replacement.end + currentLineOffset));
              originalFileWorkingContents = originalFileWorkingContents.substr(0, replacement.start + currentLineOffset);
