...
class PolymerRenamerStream extends stream.Transform {
  /**
   * @param {{
   *     typeNameLookup: (function(string):string|undefined),
   *     outputFilename: string|undefined
   *   }=} options
   */
  constructor(options = {}) {
    super({objectMode: true});
    this.typeNameLookup = options.typeNameLookup;
  ...
  _flush(cb) {
    ...
    parseElements(this.typeNameLookup, projectRoot, ...this._files)
        .then(output => {
          this.push(new gutil.File({
            base: projectRoot,
            path: this.outputFilename || this._files[0].path,
            contents: Buffer.from(output, 'utf8')
          }));
          cb();
        })
        .catch(err => {
