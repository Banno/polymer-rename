...
class PolymerRenamerStream extends stream.Transform {
  /**
   * @param {function(string):string=} customTypeNameLookup
   */
  constructor(customTypeNameLookup) {
    super({objectMode: true});
    this.typeNameLookup = customTypeNameLookup;
  }
  ...
  _transform(file, enc, cb) {
    ...
    this.push(new gutil.File({
      path: outputPath,
      contents: new Buffer(parseElements(file.contents.toString(), this.typeNameLookup))
    }));
 ...
 * @param {function(string):string=} customTypeNameLookup
 */
module.exports = function(customTypeNameLookup) {
  return new PolymerRenamerStream(customTypeNameLookup);
};
