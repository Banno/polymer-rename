...
function parsePolymerElements(polymerTemplate) {
  let document = parse5.parse(polymerTemplate, {locationInfo: true});
  let polymerElements = parsePolymerElements.walk(document, polymerTemplate);
...
parsePolymerElements.walk = function(document, template) {
  let polymerElements = [];
  domWalker.preorder(document, (value, key, parent) => {
    if (value.tagName === 'dom-module') {
      ...
      polymerElements.push(new PolymerElementInfo(value, id.value, template));
    }
  });
