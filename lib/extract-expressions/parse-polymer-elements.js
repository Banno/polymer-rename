'use strict';
const PolymerElementInfo = require('./polymer-element-info');
...
/**
 * @param {(function(string):string|undefined)|undefined|null} customTypeNameLookup function
 * @param {string} projectRoot
 * @param {...(string|Vinyl)} files - either path names or Vinyl files
 * @returns {Promise<string>}
 */
function parsePolymerElements(customTypeNameLookup, projectRoot, ...files) {
  ...
  return analyzePromises
      .then(data => {
        ...
        domModules.forEach(domModule => {
          ...
          polymerElements.add(
              new PolymerElementInfo(domModule.node, id, data.htmlDocuments.get(domModule.sourceRange.file),
                  customTypeNameLookup, renameableElementProps));
        });
        ...
        polymerElements.forEach(elem => {
          ...
          //output.push('{', `/** @this {${elem.typeName}} */ const fn = function() {`);
          output.push(outputPolymerExpr(elem, ''));
          output.push(`}).call(/** @type {!${elem.typeName}} */ (document.createElement("${elem.tagName}")))`);
          //output.push('};', `fn.call(/** @type {!${elem.typeName}} */ (document.createElement("${elem.tagName}")))`,
          //    '}');
        });
...
function outputPolymerExpr(expression, indent) {
  const output = [];
  if (!(expression instanceof PolymerElementInfo)) {
    output.push(expression.toString(indent));
  }
  if (expression.subExpressions) {
    expression.subExpressions.forEach(item => {
      output.push(outputPolymerExpr(item, indent + '  '));
    });
  }

  if (!(expression instanceof PolymerElementInfo) && expression.subExpressions) {
    output.push(expression.toClosingString(indent));
  }
  return output.join('\n');
}
