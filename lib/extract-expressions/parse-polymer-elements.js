...
ignoredWarnings.add('unknown-superclass');
      ...
      .then(data => {
        ...
        features.forEach(feature => {
          if (feature instanceof PolymerElement && feature.tagName) {
            ...
            // console.error(feature.tagName, feature.className);

            renameableElementProps.set(
                feature.tagName,
                feature);

            const warnings = feature.warnings.filter(warning => warning.code !== 'unknown-superclass');
        ...
        domModules.forEach(domModule => {
          ...
          polymerElements.add(
              new PolymerTemplateExpressions(domModule.node, id, data.htmlDocuments.get(domModule.sourceRange.file),
                  typeNamesByTagName, renameableElementProps, typeNameLookup));
        });
