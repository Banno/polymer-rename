...
class RenameableRepeat extends BaseExpression {
  ...
  traverseRenameableChildren(renameable) {
    ...
    if (renameable instanceof RenameableMethod) {
      for (let i = 0; i < renameable.args.length; i++) {
        renameable.args[i] = this.fixRenameableSymbolReference(renameable.args[i]);
      }
      return renameable;
    } else {
      return this.fixRenameableSymbolReference(renameable);
    }
  }

  fixRenameableSymbolReference(renameable) {
  ...
  toString(indent) {
    ...
    let itemsName = this.parent ? this.parent.getSymbolQualifiedName(this.items) : ('this.' + this.items);
