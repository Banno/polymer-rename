...
class PolymerTemplateExpressions {
  ...
  constructor(domModule, tagName, elementProperties, typeNameLookup, placeholderPrefix) {
    ...
    treeWalker(this.walkDom.bind(this))
        .walk(
            this.domModule.isInline ? this.domModule.parsedDocument.ast: this.domModule.node,
            this.visitNodePre,
            this.visitNodePost,
            this);
  }
  ...
  lookupProperty(propName) {
    ...
    for (let i = this.scopeStack.length - 1; i >= 0; i--) {
      ...
      let elementTypeName = scope.tagName === 'dom-repeat' ? 'Polymer.DomRepeat' : this.typeNameLookup(scope.tagName);
  ...
  visitNodePre(node) {
    ...
    if (this.domModule.isInline && node.tagName === 'body' && !this.rootTemplate_) {
      this.rootTemplate_ = node;
      return;
    } else if (!this.domModule.isInline && node.tagName === 'template' && !this.rootTemplate_) {
      this.rootTemplate_ = node;
      return;
  ...
  visitDomRepeat(node, attributeExpressions) {
    ...
    const tagRange = getDocument(this.domModule).sourceRangeForStartTag(node);
    let domRepeatable = new DomRepeatExpr(
        getDocument(this.domModule).sourcePositionToOffset(tagRange.start),
        getDocument(this.domModule).sourcePositionToOffset(tagRange.end),
        item,
        aliasProperty,
        indexProperty);
  ...
  getExpressionsForAttribute(node, attr) {
    ...
    if (this.elementProperties.has(getNodeTagName(node))) {
      const attributeNameRange = getDocument(this.domModule).sourceRangeForAttributeName(node, attr.name);
      ...
      if (isAttributeRenameable) {
        expressions.push(
            new AttributeExpr(
                this.url,
                this.getPlaceholderProperty(),
                getDocument(this.domModule).sourcePositionToOffset(attributeNameRange.start),
                getDocument(this.domModule).sourcePositionToOffset(attributeNameRange.end) +
                    attrLocEndOffset,
                getNodeTagName(node),
    ...
    if (isChangedEvent && this.isAttributeRenamable(getNodeTagName(node), attrName)) {
      ...
      if (elementProps) {
        ...
        if (propInfo) {
          ...
          const attributeNameRange = getDocument(this.domModule)
              .sourceRangeForAttributeName(node, attr.name);
          const attrNameStart = getDocument(this.domModule)
  ...
  getAttributeValue(node, attr) {
    let valueRange = getDocument(this.domModule)
        .sourceRangeForAttributeValue(node, attr.name, true);

    if (!valueRange && attr.value) {
      const attrRange = getDocument(this.domModule)
          .sourceRangeForAttribute(node, attr.name);
      if (!attrRange) {
        return;
      }
      ...
      const docLines = getDocument(this.domModule).contents.split('\n');
      if (this.domModule.isInline) {
        for (let i = 0; i < this.domModule.sourceRange.start.line; i++) {
          docLines.unshift('');
        }
        docLines[this.domModule.sourceRange.start.line] =
            ''.padStart(this.domModule.sourceRange.start.column, ' ') +
            docLines[this.domModule.sourceRange.start.line];
      }
    ...
    return Object.assign({}, attr, {
      start: getDocument(this.domModule).sourcePositionToOffset(valueRange.start),
      end: getDocument(this.domModule).sourcePositionToOffset(valueRange.end) - 1,
    });
  }
  ...
  visitTextNode(node) {
    let callCount = 0;
    let dataBindingInfo;
    const textRange = getDocument(this.domModule)._sourceRangeForNode(node);
    if (textRange === undefined) {
      return;
    }
    let start = getDocument(this.domModule).sourcePositionToOffset(textRange.start);
    let end = getDocument(this.domModule).sourcePositionToOffset(textRange.end);
