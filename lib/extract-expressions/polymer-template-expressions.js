...
class PolymerTemplateExpressions {
   ...
   * for renaming
   *
   * @param {!Element} domModule HTML template node which is a dom-module
   * @param {string} tagName of the element
   * @param {string} originalHtml contents of the full HTML document containing the dom-module
   ...
   *     type name
   */
  constructor(domModule, tagName, originalHtml, elementProperties, typeNameLookup) {
  ...
  getExpressionsForAttribute(node, attr) {
    ...
    if (this.elementProperties.has(node.tagName)) {
      isAttributeRenameable = this.isAttributeRenamable(node.tagName, attrName);
   ...
   *
   * @param {Object} tagLocationInfo location data for the host tag
   * @param {Object} attr information provided by the parser
   * @return {{
   *     name: string,
   *     value: string,
  ...
  getAttributeValue(tagLocationInfo, attr) {
    ...
    let startIndex = attrLocationInfo.startOffset;
    let endIndex = attrLocationInfo.endOffset;
  ...
  visitTextNode(node) {
    let callCount = 0;
    let dataBindingInfo;
    let start = node.__location.startOffset;
    let end = node.__location.endOffset - 1;
  ...
  expressionFromPropertyName(propName, startIndex, endIndex, isMethod = false) {
    ...
    if (!propertyInfo) {
      ...
      return new IdentifierExpr(startIndex, endIndex + 1, propName, true);
    }
