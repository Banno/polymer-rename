...
class PolymerTemplateExpressions {
   ...
   * @param {Map<string, string>} typeNamesByTagName given a tag name, returns the
   *     type name
   * @param {Map<string, Array<string>} elementProperties
   */
  constructor(domModule, tagName, originalHtml, typeNamesByTagName, elementProperties) {
    this.domModule = domModule;
    this.tagName = tagName;

    /** @type {Map<string, string>} */
    this.typeNamesByTagName = typeNamesByTagName;
  ...
  typeNameLookup(tagName) {
    let typeName = this.typeNamesByTagName.get(tagName);
    if (typeName) {
      return typeName;
    }

    // TODO: Warn that type wasn't found
  ...
  lookupMethod(propName) {
    ...
    if (!method) {
      method = this.lookupProperty(propName);
      if (method) {
        return method;
      }
      console.error(feature, propName);
    }
  ...
  visitAttribute(node, attr) {
    ...
    if (this.elementProperties.has(node.tagName)) {
      ...
      let changedEventParts = PolymerTemplateExpressions.CHANGED_EVENT_ATTRIBUTE.exec(attrName);
      ...
      if (changedEventParts && !isAttributeRenameable) {
        attributeNameSubstring = changedEventParts[1];
        attrNameStart += 3; // skip the 'on-' prefix
        isAttributeRenameable = this.isAttributeRenamable(node.tagName, attributeNameSubstring, true);
      }
  ...
  processDataBindingExpression(dataBoundExpression, startIndex, endIndex, elementTagName,
    ...
    // each argument
    let methodParts = PolymerTemplateExpressions.METHOD_MATCH_EXPR.exec(dataBoundExpression);
    if (methodParts !== null) {
      let methodPropInfo = this.lookupMethod(methodParts[1]);
      ...
      if (elementTypeName && elementTagName && elementAttribute) {
        ...
        let attributeProperty = elementAttribute.replace(/[A-Z]/g, PolymerTemplateExpressions.camelCaseToHyphenatedReplacement);
        expressions.push(
  ...
  expressionInfoFromPropertyName(startIndex, endIndex, propName) {
    ...
    if (!data.propertyInfo) {
      data.propertyInfo = this.lookupMethod(propName);
    }
...
PolymerTemplateExpressions.CHANGED_EVENT_ATTRIBUTE = /^on-(.*)-changed$/;
