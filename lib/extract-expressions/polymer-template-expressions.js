...
class PolymerTemplateExpressions {
  ...
  visitDomRepeat(node, attributeExpressions) {
    ...
    if (!items || items.length === 0) {
      throw new Error(`Unable to locate dom-repeat items property 'items' in template ${this.scopeStack[0].tagName}`);
    }
    items = items[0];
  ...
  getExpressionsForEventAttribute(node, attr) {
    ...
    if (propInfo) {
      ...
      console.error(`unable to find method ${attrValue.value}`);
      // TODO: Handle error for method not found
    }

    // on-prop-changed event
    if (isChangedEvent && this.isAttributeRenamable(node.tagName, attrName)) {
      const propName = attrName.replace(/-/g, PolymerTemplateExpressions.hyphenatedToCamelCaseReplacement);
      const elementProps = this.elementProperties.get(node.tagName);
      if (elementProps && elementProps.properties.find(prop => prop.name === propName)) {
        const propInfo = elementProps.properties.find(prop => prop.name === propName);
        if (propInfo) {
          ...
          console.error(`unable to find property ${propName}`);
        }
      } else {
        console.error(`unable to find property ${propName}`);
      }
  ...
  parseDataBinding(dataBoundExpression) {
    ...
    if (methodParts !== null) {
      ...
      const methodInfo = this.lookupMethod(methodParts[1]);
      if (methodInfo === null) {
  ...
  expressionFromPropertyName(propName, startIndex, endIndex, isMethod = false) {
    ...
    if (!propertyInfo) {
      throw new Error(`Unable to locate ${isMethod ? 'method' : 'property'} '${propName}' in template for '${this.scopeStack[0].tagName}'`);
    }
