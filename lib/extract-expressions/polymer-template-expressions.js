...
class PolymerTemplateExpressions {
  ...
  getAttributeValue(node, attr) {
    let valueRange = this.domModule.astNode.containingDocument
        .sourceRangeForAttributeValue(node, attr.name, true);

    if (!valueRange && attr.value) {
      const attrRange = this.domModule.astNode.containingDocument
      ...
      const docLines = this.domModule.astNode.containingDocument.contents.split('\n');
