...
class PolymerTemplateExpressions {
   ...
   * for renaming
   *
   * @param {Object} domModule feature from polymer-analyzer
   ...
   *     type name
   */
  constructor(domModule, tagName, elementProperties, typeNameLookup, placeholderPrefix) {
    this.domModule = domModule;
    this.url = domModule.astNode.containingDocument.url;
    ...
    treeWalker(this.walkDom.bind(this)).walk(this.domModule.node,
  ...
  lookupProperty(propName) {
    ...
    for (let i = this.scopeStack.length - 1; i >= 0; i--) {
      ...
      const elementTypeName = scope.tagName === 'dom-repeat' ? 'Polymer.DomRepeat' : this.typeNameLookup(scope.tagName);
  ...
  visitDomRepeat(node, attributeExpressions) {
    ...
    const tagRange = this.domModule.astNode.containingDocument.sourceRangeForStartTag(node);
    let domRepeatable = new DomRepeatExpr(
        this.domModule.astNode.containingDocument.sourcePositionToOffset(tagRange.start),
        this.domModule.astNode.containingDocument.sourcePositionToOffset(tagRange.end),
        item,
        aliasProperty,
        indexProperty);
  ...
  getDomRepeatSortImplicit(node, attribute) {
    ...
    if (attrValue) {
      ...
      if (propInfo) {
        expressions.push(
            new IdentifierExpr(
                this.url, this.getPlaceholderProperty(), attrValue.start, attrValue.end, attrValue.value));
      }
    }
      ...
      if (attributeInfo) {
        ...
        if (!property) {
          property = {
            name: attributeValue,
            base: attributeValue,
            elementTypeName: 'Polymer.DomRepeat',
  ...
  getExpressionsForAttribute(node, attr) {
    ...
    if (this.elementProperties.has(getNodeTagName(node))) {
      const attributeNameRange = this.domModule.astNode.containingDocument.sourceRangeForAttributeName(node, attr.name);
      ...
      if (isAttributeRenameable) {
        expressions.push(
            new AttributeExpr(
                this.url,
                this.getPlaceholderProperty(),
                this.domModule.astNode.containingDocument.sourcePositionToOffset(attributeNameRange.start),
                this.domModule.astNode.containingDocument.sourcePositionToOffset(attributeNameRange.end),
                getNodeTagName(node),
    ...
    if (isChangedEvent && this.isAttributeRenamable(getNodeTagName(node), attrName)) {
      ...
      if (elementProps) {
        ...
        if (propInfo) {
          ...
          const attributeNameRange = this.domModule.astNode.containingDocument
              .sourceRangeForAttributeName(node, attr.name);
          const attrNameStart = this.domModule.astNode.containingDocument
              .sourcePositionToOffset(attributeNameRange.start) + 3;
          expressions.push(
              new AttributeExpr(
                  this.url,
                  this.getPlaceholderProperty(),
  ...
  getAttributeValue(node, attr) {
    const valueRange = this.domModule.astNode.containingDocument
        .sourceRangeForAttributeValue(node, attr.name, true);
    ...
    return Object.assign({}, attr, {
      start: this.domModule.astNode.containingDocument.sourcePositionToOffset(valueRange.start),
      end: this.domModule.astNode.containingDocument.sourcePositionToOffset(valueRange.end) - 1,
    });
  }
  ...
  visitTextNode(node) {
    let callCount = 0;
    let dataBindingInfo;
    const textRAnge = this.domModule.astNode.containingDocument._sourceRangeForNode(node);
    let start = this.domModule.astNode.containingDocument.sourcePositionToOffset(textRAnge.start);
    let end = this.domModule.astNode.containingDocument.sourcePositionToOffset(textRAnge.end);
  ...
  expressionFromPropertyName(propName, startIndex, endIndex, isMethod = false) {
    ...
    if (!propertyInfo) {
      ...
      return new IdentifierExpr(
          this.url, this.getPlaceholderProperty(), startIndex, endIndex, propName, true);
    }

    if (propertyInfo.elementTypeName === 'Polymer.DomRepeat') {
      if (!propertyInfo.renameable && propertyInfo.base !== propertyInfo.name) {
        ...
        return new IdentifierExpr(
            this.url, this.getPlaceholderProperty(), startIndex, endIndex, propertyInfo.name, false);
      }
      return new DomRepeatIdentifierExpr(
