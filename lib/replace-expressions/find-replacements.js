...
class FindReplacementExpressions {
  ...
  constructor(src) {
    ...
    this.replacementsPerFile = new Map();
    this.replacements.forEach(replacement => {
      if (!this.replacementsPerFile.get(replacement.url)) {
        this.replacementsPerFile.set(replacement.url, []);
      }
      this.replacementsPerFile.get(replacement.url).push(replacement);
    });
  }
  ...
  visit(node) {
    if (node.type === 'CallExpression' &&
      ...
      if (node.callee.property.name === 'attribute') {
        this.visitAttributeRenamable(args[0].value, args[1].value, args[2].value, args[3].value, args[4], args[5]);
      } else if (node.callee.property.name === 'domRepeatObserve') {
        this.visitObserveRenamable(args[0].value, args[1].value, args[2], args[3], args[4]);
      } else {
        this.visitBasicRenamable(args[0].value, args[1].value, args[2].value, args[3].value, args[4], args[5], args[6]);
      }
    }
  }
   ...
   */
  visitAttributeRenamable(url, placeholder, start, end, baseProp, prop) {
   ...
   */
  visitObserveRenamable(url, placeholder, start, end, prop, baseProp, basePropName) {
    let propParts = FindReplacementExpressions.getPropertyString(prop).split('.');
   ...
   * @param {string} basePropName potentially quoted base identifier for the expression
   */
  visitBasicRenamable(url, placeholder, start, end, prop, baseProp, basePropName) {
    let propParts = FindReplacementExpressions.getPropertyString(prop).split('.');
